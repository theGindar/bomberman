...
if len(os.listdir("./agent_code/my_agent/saved_models")) != 0:
    ...
    policy_net.load_state_dict(
        torch.load("./agent_code/my_agent/saved_models/krasses_model.pt", map_location=torch.device('cpu')))
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if state_to_features(old_game_state) != None:
        self.memory.push(state_to_features(old_game_state).to(device), self_action,
                         state_to_features(new_game_state).to(device), reward)
