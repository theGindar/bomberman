...
# DQN Model
dqn_hidden_state = dqn_cell_state = None
target_hidden_state = target_cell_state = None

dqn: LSTMDQN = LSTMDQN(env.action_space)

# For Optimization
dqn_hidden_state, dqn_cell_state = dqn.init_states()
target_hidden_state, target_cell_state = dqn.init_states()

# For Training Play
train_hidden_state, train_cell_state = dqn.init_states()

# For Validation Play
test_hidden_state, test_cell_state = dqn.init_states()

# DQN Target Model
target: LSTMDQN = copy.deepcopy(dqn)

if len(os.listdir("./agent_code/lstm_agent/saved_models")) != 0:
    print('loading existing model...')
    dqn.load_state_dict(torch.load("./agent_code/lstm_agent/saved_models/krasses_model.pt", map_location=torch.device('cpu')))
